...
b8 Sere_InitLogging(void)
{
    ...
    log_file_handle = Sere_OpenFile("console.log", SERE_FILEMODE_WRITE, SERE_FALSE);
    if (!log_file_handle || !log_file_handle->is_valid)
    {
        Sere_PlatformConsoleWriteError("Failed to open console.log for writing.", SERE_LOG_LEVEL_ERROR);
        return SERE_FALSE;
    }
...
SERE b8 Sere_LoggingInitialized(void)
{
...
static void Sere_AppendToLogFile(const char *message)
{
    ...
    u64 written = 0;
    if (!Sere_FileWrite(log_file_handle, length, message, &written) || written != length)
    {
        Sere_PlatformConsoleWriteError("ERROR writing to console.log", SERE_LOG_LEVEL_ERROR);
    }
}
