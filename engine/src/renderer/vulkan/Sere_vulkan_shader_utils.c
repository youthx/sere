...
b8 Sere_CreateShaderModule(Sere_VulkanContext *context, const char *name,
                           const char *type_str,
  ...
  Sere_StrFormat(file_name, sizeof(file_name), "./bin/assets/shaders/%s.%s.spv",
                 name, type_str);
  ...
  SERE_VK_CHECK(vkCreateShaderModule(context->device.logical_device,
                                     &shader_stages[stage_index].create_info, 0,
  ...
  if (file_buffer) {
    Sere_Free(file_buffer, sizeof(u8) * size, SERE_MEMORY_TAG_STRING);
    file_buffer = 0;
  }
